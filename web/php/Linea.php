...
if(permitido("fun1001", $_SESSION['codigo'])==true)
{
    ...
    if($funcion == "ListarLinea"){
        ...
        $band = false;
        if($_GET['buscarlineax'] != null)
        {
            if($band == false) {
                $extras .= " lin.codigo LIKE '%".$_GET['buscarlineax']."%'";
                $band = true;
            }
            else {
                $extras .= " AND lin.codigo  LIKE '%".$_GET['buscarlineax']."%'";
            }
        }
        if($_GET['buscarlinea'] != null)
        {
            if($band == false) {
                $extras .= " lin.idlinea LIKE '".$_GET['buscarlinea']."'";
                $band = true;
            }
            else {
                $extras .= " AND lin.idlinea LIKE '".$_GET['buscarlinea']."'";
            }
        }
        ...
        {
            if($band == false) {
                $extras .= " lin.idestilo LIKE '".$_GET['buscarestilo']."' ";
                $band = true;
            }
            else {
                $extras .= " AND lin.idestilo LIKE '".$_GET['buscarestilo']."'";
            }
        }
