...
function findUltimoNumeroPedidoMarca($idmarca,$return){
    $sql = "SELECT
  num.idnumero,
  num.idmarca,
  num.numero
...
function ejecutarConsultaSQLBeginCommit($sql)
{
    if($sql!=NULL)
    {
        ...
        { $link->consulta("commit");
            ...
            GenerarErrorLog("error mysql", "Haydee", $sqll, "Ok", "no", "si", "./");
            return true;
        }
        else
        { $link->consulta("rollback");
            $sqll = ReturnConsultas($sql)."-->".$link->getError;
            GenerarErrorLog("error", "Haydee", $sqll, "Error", "no", "si", "./");
            return false;
        }
        $link->cerrar();
    }
